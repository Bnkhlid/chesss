...
class ChessGame {
  ...
  movePiece(fromRow, fromCol, toRow, toCol, isRemote = false) {
    ...
    // Romantic Easter Egg - Random chance on move (10%)
  ...
  renderBoard() {
    ...
    for (let row = rowStart; row !== rowEnd; row += rowStep) {
      for (let col = colStart; col !== colEnd; col += colStep) {
        ...
        // Use actual board coordinates (row, col) to get the piece
        const piece = this.board[row][col];
        if (piece) {
    ...
    // Replace board content in one atomic operation
    boardEl.innerHTML = "";
    boardEl.appendChild(fragment);

    // Animation disabled to prevent visual flicker
    /*
    if (this.lastMove) {
      try {
  ...
  isRomanticName(name) {
    ...
    return targets.some(t => lower.includes(t)) || lower.includes("♕");
  }
  ...
  handlePowerClick(item) {
    ...
    if (this.canActivatePower(this.currentPlayer, power)) {
      ...
      if (power === "queen-rush") {
        ...
        this.addChatMessage(
          "System",
          `👑 ${this.currentPlayer === "white" ? "White" : "Black"} activated Queen Rush! All pieces move like queens this turn.`,
        );
  ...
  makeAIMove() {
    ...
    // For very deep searches, Web Workers would be better, but depth 3 is fine here.
    setTimeout(() => {
